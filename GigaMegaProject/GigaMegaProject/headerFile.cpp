...
void deleteHistory() {
	...
	while (1) {
		cout << "Podaj indeks do usuniecia (0 - " << dataCounter-1 << ") lub wpisz N jesli chcesz anulowac" << endl;
		cout << "Wybor: ";
		cin >> indexToDelete;
		if (indexToDelete == "N" || indexToDelete == "n") {
			isCancelled = true;
			break;
		}
	...
	if (!isCancelled) {
		...
		cout << "Anulowano usuwanie wpisu z historii." << endl;
	}
}
...
void modifyHistory() {
	...
	while (1) {
		cout << "Podaj indeks do modyfikacji (0 - " << dataCounter-1 << ") lub wpisz N jesli chcesz anulowac" << endl;
		cout << "Wybor: ";
		cin >> indexToModify;
		if (indexToModify == "N" || indexToModify == "n") {
			isCancelled = true;
			break;
		}
	...
	if (!isCancelled) {
		...
		cout << "Anulowano modyfikacje wpisu z historii." << endl;
	}
}
	...
	while (1) {
		...
		if (checkSpelling(1, choice)) {
			...
			if (option == -1) {
				...
				cout << "Niepoprawna opcja, sproboj jeszcze raz." << endl;
				clearTheScreenAfterInteraction();
			}
		}
		else {
